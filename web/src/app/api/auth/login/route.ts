...
export async function POST(request:Request){
 ...
 let rateKeys:string[]=[];
 try{rateKeys=await loginRateLimitKeys(request,sessionSecret,email);if(await consumeDashboardLoginAttempts(rateKeys))return Response.json({error:"로그인 시도가 너무 많습니다. 잠시 후 다시 시도해 주세요."},{status:429,headers:{...NO_STORE_HEADERS,"Retry-After":"900"}})}catch(error){console.error(JSON.stringify(authInfrastructureTelemetry("LOGIN_LIMITER_CONSUME",error)));return Response.json({error:AUTH_INFRASTRUCTURE_MESSAGE,code:"AUTH_INFRASTRUCTURE_UNAVAILABLE"},{status:503,headers:NO_STORE_HEADERS})}
 if(!isValidEmail(email))return Response.json({error:AUTH_REJECTED_MESSAGE},{status:401,headers:NO_STORE_HEADERS});
 let subjectId:string|null=null;
 try{subjectId=await findDashboardSubjectByEmail(email)}catch(error){console.error(JSON.stringify(authInfrastructureTelemetry("ALLOWLIST_LOOKUP",error)));return Response.json({error:AUTH_INFRASTRUCTURE_MESSAGE,code:"AUTH_INFRASTRUCTURE_UNAVAILABLE"},{status:503,headers:NO_STORE_HEADERS})}
 if(!subjectId)return Response.json({error:AUTH_REJECTED_MESSAGE},{status:401,headers:NO_STORE_HEADERS});
 try{await clearDashboardLoginAttempts(rateKeys)}catch(error){console.error(JSON.stringify(authInfrastructureTelemetry("LOGIN_LIMITER_CLEAR",error)))}
