...
vi.mock("@/lib/server/db",()=>({authorizeDashboardSubject:authMock}));
import {proxy} from "@/proxy";
const SUBJECT_ID="49caafcd-f6c5-4d79-92bd-6f4cd968cf25",SESSION_SECRET="0123456789abcdef0123456789abcdef";
beforeEach(()=>{process.env.DASHBOARD_SESSION_SECRET=SESSION_SECRET;authMock.mockResolvedValue(true)});
...
describe("dashboard proxy",()=>{
 ...
 it("caches an approved signed subject for at most 30 seconds without hiding route timing",async()=>{vi.useFakeTimers();vi.setSystemTime(new Date("2026-08-21T00:00:00Z"));const token=await createSessionToken(SUBJECT_ID,SESSION_SECRET),request=new NextRequest("https://example.com/api/index",{headers:{Cookie:`${SESSION_COOKIE}=${token}`}});const first=await proxy(request);expect(first.status).toBe(200);expect(first.headers.get("x-cre-authz-ms")).toMatch(/^\d+\.\d$/u);expect(first.headers.get("server-timing")).toBeNull();authMock.mockResolvedValue(false);vi.advanceTimersByTime(29_999);expect((await proxy(request)).status).toBe(200);expect(authMock).toHaveBeenCalledTimes(1);vi.advanceTimersByTime(1);expect((await proxy(request)).status).toBe(401);expect(authMock).toHaveBeenCalledTimes(2)});
